...
### 1.8 Fob button feedback and spam-safety (instant-ack + lockout on any delayed action)
...
This is enabled by **decoupling the button's visual from the slow action**: the firmware sets the *working* state the instant the press registers — *before* it begins the camera commands — so the acknowledgement is immediate, whether or not the action has finished. There is **no command queue and no background worker running the START on the fob's behalf**; the camera commands run inline and are fast because they are fire-and-forget (the fob does not wait for a reply). What *does* run on a **separate core** is the fob's continuous **camera-presence sensing**, so a camera dropping mid-take is still noticed while a take is open. Spam-safety is further hardened by writing the **durable take number to flash before the counter advances**, so a crash or battery swap can't lose or reuse a number. Both layers live entirely in the coordinator's UI + core (they validate the swappable-UI seam — a different screen reimplements the UI layer against the same core guarantee). See the "impatient operator spams START" walkthrough for the step-by-step.

**This is a general rule, not a START-only one (observed across runs on the rig).** START is the worst case (the ~3 s pipeline re-init), but **every fob control whose action has a perceptible delay** — anything where the fob does network/telnet/OSC work between the touch and the result and the operator could be left wondering whether the tap registered — gets the **same instant-ack + working-state + lockout** treatment, backed by the **same core "valid only from the right state, extra taps dropped" guarantee**. Concretely this covers STOP (finalize/flush latency), and any settings/sign-in/confirm action that round-trips to the camera network or the god's-view server. The principle: **the visual acknowledgement is decoupled from the slow action on every button, and no button can be double-fired mid-action** — so an operator never has to guess whether a press landed, regardless of which control it was. A button whose action is genuinely instant needs no working-state; the rule is scoped to "has a perceptible delay."
...
### 1.10 The god's-view dashboard — the operator drill-down

The god's-view is the operational dashboard for the people running collection (managers on-site, founders in SF). Its screen design is a **three-level drill-down**: a list of **all operators with name and status**; clicking an operator shows **their most recent episodes** (the last ten we have from them); clicking a video shows **the footage with that episode's metadata alongside it** (video on one side, the identity/timing/outcome fields on the other). This is a **view over the operational store and the rendered footage** — it is a reader, not a new system, and it shares the operational tier's data with the spot-check viewer (§1.9). In all likelihood it is *the same* dashboard: one operations console with a quality/spot-check view and this operator drill-down view, both reading the same store and the same single Hades render, rather than two separate apps. A prototype of this family already exists (Victor's QA viewer, which browses footage by operator and episode and transcodes clips on demand into a bounded cache); Eunomia unifies it into the console layer and builds it against the contract.

The design has two halves with very different readiness, and it is worth being honest about which is which.

- **The historical half — last-N-episodes and the video-plus-metadata view — is the buildable part and is not blocked.** It reads data that already lands: episodes drain to the on-site store and render on the analytical box, and the metadata is exactly the sidecar and operational-episode records the contract already defines. "The last ten episodes from this operator" is a query keyed on the operator, resolved through the **session** (the dashboard pivots on *operator*, which is precisely why operator identity is kept distinct from kit — see §1.7 and the operational model; an operator uses any kit, and the session records who used what when). "Show this video with its metadata" is the rendered clip (preferred) or the fresh-window raw fallback (§1.9) shown next to that episode's fields. This half follows naturally once the operational store and the render are populated.

- **The live half — the operators list updating with status — is real as a design but inherits the single-radio constraint (§1.4).** The fob cannot stream live telemetry while it is hosting the camera network: to push anything it would have to leave that network, and leaving it mid-session drops the cameras. So "live telemetry per operator" has no continuous transport on today's single-radio hardware. The honest version of the live strip is therefore the same **near-real-time, state-transition-plus-last-seen** freshness the god's-view already promises (§1.4) — "who is signed in, who is recording, who was last seen N minutes ago, whose last episode landed when" — derived from the operational store as cards and episodes arrive, rather than a live battery/SD stream. Richer live telemetry (battery, storage) becomes possible only with a transport that does not tear down the camera network — a second radio, or another vantage — which is the same hardware question as giving the fob a reliable clock; both are "things the fob cannot do while hosting the AP," and both wait on that. The dashboard should present the live strip as freshness it can actually guarantee, and treat battery/storage as best-effort shown only when a real uplink exists.

The dashboard is reachable over the tailnet from both Mexico and SF (like the spot-check viewer), and it is a **later build** — a consumer that comes after the coordinator firmware and after the ingest/quality path is populating the store. It is captured here so that build starts from a real screen design rather than a blank page, and so the two dependencies are explicit: the historical half waits on the store and render being populated; the live half waits on a camera-network-safe uplink for anything beyond last-seen freshness.
